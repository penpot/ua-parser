...
const NAVIGATOR = (hasWindow && window.navigator) ? window.navigator : undefined;

///////////
// Helper
//////////

function extend (defaultRgx, extensions) {
  var mergedRgx = {};
...
function has (str1, str2) {
  if (isObject(str1) && str1.length > 0) {
    for (var i in str1) {
...
function isExtensions (obj, deep) {
  for (var prop in obj) {
...
function strip (pattern, str) {
  return isString(str) ? str.replace(pattern, EMPTY) : str;
}

function trim (str, len) {
  str = strip(/^\s\s*/, String(str));
  return isUndefined(len) ? str : str.substring(0, len);
}
...
function strMapper (str, map) {
  ...
  for (var i in map) {
    ...
    if (isObject(map[i]) && map[i].length > 0) {
...
function parsePart (itemType, ua, rgxMap) {

  const fields = partFields[itemType];

  const data = {};
  for (let i = 0; i < fields.length; i++) {
    data[fields[i]] = undefined;
  }

  rgxMapper.call(data, ua, rgxMap);

  if (itemType === BROWSER) {
    data[MAJOR] = majorize(data[VERSION]);
  }
  if (itemType === OS) {
    if (data[NAME] == 'iOS' && data[VERSION]) {
      if (/^1[89][^\d]/.exec(data[VERSION])) {
        var realVersion = /\) Version\/((\d+)[\d\.]*)/.exec(ua);
        if (realVersion && parseInt(realVersion[2], 10) >= 26) {
          data[VERSION] = realVersion[1];
        }
      }
    }
  }

  Object.defineProperty(data, '_type', {
    value: itemType,
    enumerable: false
  });
...
function is (data, strToCheck) {
  const itemType = data._type;
  const ignoreProps = partIgnoreInIs[itemType] || 0;
  const stripRgx = partStripInIs[itemType] || 0;
  let match = false;
  for (var i in data) {
    if (data.hasOwnProperty(i) && !has(ignoreProps, i) && lowerize(stripRgx ? strip(stripRgx, data[i]) : data[i]) == lowerize(stripRgx ? strip(stripRgx, strToCheck) : strToCheck)) {
      match = true;
      if (strToCheck != 'undefined') break;
    } else if (strToCheck == 'undefined' && match) {
      match = !match;
      break;
    }
  }
...
function toString (data) {
  const itemType = data._type;
  const fields = partToStringFields[itemType] || 0;
  var str = EMPTY;
  for (var i in fields) {
    if (!isUndefined(data[fields[i]])) {
      str += (str ? ' ' : EMPTY) + data[fields[i]];
    }
  }
...
class ParseResult {
  constructor(userAgent, regexMap) {
    this._userAgent = userAgent;
    this._regexMap = regexMap;
  }

  getBrowser() {
    return parsePart(BROWSER, this._userAgent, this._regexMap[BROWSER]);
  }

  getCPU() {
    return parsePart(CPU, this._userAgent, this._regexMap[CPU]);
  }

  getDevice() {
    return parsePart(DEVICE, this._userAgent, this._regexMap[DEVICE]);
  }

  getEngine() {
    return parsePart(ENGINE, this._userAgent, this._regexMap[ENGINE]);
  }

  getOS() {
    return parsePart(OS, this._userAgent, this._regexMap[OS]);
  }
...
function parse (ua, extensions) {
  const userAgent = isString(ua) ? trim(ua, UA_MAX_LENGTH) :
      ((NAVIGATOR && NAVIGATOR.userAgent) ? NAVIGATOR.userAgent : EMPTY);

  const regexMap = extensions ? extend(defaultRegexes, extensions) : defaultRegexes;

  return new ParseResult(userAgent, regexMap);
}

const BrowserEnum = enumerize([NAME, VERSION, MAJOR, TYPE]);
const CpuEnum = enumerize([ARCHITECTURE]);
const DeviceEnum = enumerize([MODEL, VENDOR, TYPE, CONSOLE, MOBILE, SMARTTV, TABLET, WEARABLE, EMBEDDED]);
const EngineEnum = enumerize([NAME, VERSION]);
const OsEnum = enumerize([NAME, VERSION]);
